...
pub mod history_path_handler {
    ...
    pub fn barchart_values(args: Vec<&str>) -> HttpResponse {
        ...
        let query: String = format!("select strftime('%H:%M', time), avg(value) from {} where time < '{}' and time > '{}'  group by strftime ('%H',time) order by strftime ('%H:%M',time)", args[0], now, six_hours_back);
